...
################# READ ###############
lines = pd.read_table('/home/ikaraman/Desktop/fra.txt', names=['eng', 'fr'])
lines = lines[0:10000]
lines.shape
lines.sample(10)
...
fr_max = np.max(lenght_list)

lenght_list = []
for l in lines.eng:
    lenght_list.append(len(l.split(' ')))
en_max = np.max(lenght_list)

encoder_input_data = np.zeros(
    (len(lines.eng), en_max),
    dtype='float32')
decoder_input_data = np.zeros(
    (len(lines.fr), fr_max),
    dtype='float32')
decoder_target_data = np.zeros(
    (len(lines.fr), fr_max, num_decoder_tokens),
    dtype='float32')
...
for seq_index in [5,2358,4378,4000, 3000, 2000, 10, 20, 1000]:
