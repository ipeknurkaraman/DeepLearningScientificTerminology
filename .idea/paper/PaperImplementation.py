...
for item in itemlist:
    ...
    for sentence in sentences:
        ...
        toxml = sentence.toxml()
        consTags = minidom.parseString(toxml).getElementsByTagName('cons')
        originalSentence = re.sub('<[^<]+>', "", toxml).replace('(','').replace(')','')
        ...
        for cons in consTags:
            innerConsTags = minidom.parseString(cons.toxml()).getElementsByTagName('cons')
            if len(innerConsTags) ==1:
                term = re.sub('<[^<]+>', "", cons.toxml()).replace('(','').replace(')','')
                ...
                if len(termElements)==1:
                    ...
                    for sentenceStartIndex in allSentenceStartIndexes:
                        ...
                        allWordIndexes.append(len(firstPartOfSentences.split()))
                   ...
                   while True:
                       # Advance location by 1.
                        location = originalSentence.find(term, location + 1)
                       # Break if not found.
                        if location == -1: break
                        allSentenceStartIndexes.append(location)
                   allWordIndexes=[]
...
for sentenceAndTag in sentencesAndTags:
    ...
    sentencesAndTagsPreprocessed.append((str(sentenceAndTag[0].translate(str.maketrans('','',string.punctuation))).lower(),sentenceAndTag[1]))
